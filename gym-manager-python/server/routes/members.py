...
@router.get("/members")
def list_members(q: str = "", status: str = "all", view: str = "all", package_id: int | None = Query(None, alias="packageId"), trainer_id: int | None = Query(None, alias="trainerId"), sort: str = "newest", page: int = Query(1, ge=1), page_size: int = Query(20, ge=10, le=100, alias="pageSize"), db: Session = Depends(get_db)):
    return members_controller.list_members(db, q=q, member_status=status, view=view, package_id=package_id, trainer_id=trainer_id, sort=sort, page=page, page_size=page_size)
