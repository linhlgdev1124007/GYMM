  ...
  const metrics = data
    ? [
        ...
        },
        {
          ...
          to: "/members?view=expiring",
        },
        {
          label: "Doanh thu tháng",
          value: money(data.metrics.revenueMonth),
          to: "/payments",
        },
        {
          label: "Công nợ",
          value: money(data.metrics.outstanding),
          to: "/members?view=debt",
        },
      ]
    : [];
  return (
    <>
      <PageHeader
        ...
        title="Dashboard"
        description="Những tín hiệu cần theo dõi trong ngày hôm nay."
      />
      <div className="metric-strip">
        {query.isLoading
          ...
          : metrics.map((metric) => (
              <Link
                className="metric hover:bg-slate-50"
                key={metric.label}
                to={metric.to}
              >
      ...
      <div className="section-grid">
        ...
        <section className="section">
          <div className="section-header">
            <div>
              <h2>Trạng thái gói tập</h2>
              <p>Phân bổ hiện tại</p>
            </div>
          </div>
          <div className="panel">
            <div className="definition-list">
              {Object.entries(data?.membershipStatus || {}).map(
                ([status, count]) => (
                  <div key={status}>
                    <dt>
      ...
      <div className="section-grid">
        <section className="section">
          <div className="section-header">
            <div>
              <h2>Hội viên cần xử lý</h2>
              <p>Hết hạn hoặc còn công nợ</p>
            </div>
            <Link to="/members">Xem hội viên</Link>
          </div>
          <DataTable
            ...
            emptyTitle="Không có vấn đề cần xử lý"
            emptyDescription="Mọi gói tập và công nợ hiện đang ổn định."
          />
        </section>
        <section className="section">
          <div className="section-header">
            ...
            columns={[
              {
                ...
                render: (row) => (
                  <div>
                    <span className="cell-primary">{row.member}</span>
                    <div className="cell-secondary">{row.code}</div>
                  </div>
                ),
              },
