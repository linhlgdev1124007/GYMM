...
export function ReportsPage() {
  ...
  const revenueColumnDefinitions = [
    ...
    { key: "basePrice", label: "Giá gốc", className: "text-right tabular-nums", render: (row) => row.type === "membership" ? money(row.basePrice) : "—" },
    { key: "discountAmount", label: "Ưu đãi", className: "text-right tabular-nums", render: (row) => row.type === "membership" ? <span className={Number(row.discountAmount || 0) > 0 ? "font-medium text-emerald-700" : "text-slate-500"}>-{money(row.discountAmount)}</span> : "—" },
    { key: "surchargeAmount", label: "Phụ thu", className: "text-right tabular-nums", render: (row) => row.type === "membership" ? <span className={Number(row.surchargeAmount || 0) > 0 ? "font-medium text-amber-700" : "text-slate-500"}>{money(row.surchargeAmount)}</span> : "—" },
    { key: "finalPrice", label: "Tổng gói", className: "text-right tabular-nums", render: (row) => row.type === "membership" ? money(row.finalPrice) : "—" },
        ...
        <div className="report-view">
          ...
          <section className="report-panel report-section-block">
            ...
            <div className="method-breakdown">
              <div><span><strong>Giá gốc gói</strong><small>{summary.membershipAdjustmentCount || 0} gói</small></span><span className="method-track"><i style={{ width: "100%" }} /></span><strong>{money(summary.membershipBaseValue)}</strong></div>
              <div><span><strong>Ưu đãi đã áp dụng</strong><small>Giảm khỏi giá gốc</small></span><span className="method-track"><i style={{ width: summary.membershipBaseValue ? `${Math.min((Number(summary.membershipDiscountAmount || 0) / Number(summary.membershipBaseValue || 1)) * 100, 100)}%` : "0%" }} /></span><strong>{money(summary.membershipDiscountAmount)}</strong></div>
              <div><span><strong>Phụ thu đã ghi nhận</strong><small>Cộng sau ưu đãi</small></span><span className="method-track"><i style={{ width: summary.membershipAdjustedValue ? `${Math.min((Number(summary.membershipSurchargeAmount || 0) / Number(summary.membershipAdjustedValue || 1)) * 100, 100)}%` : "0%" }} /></span><strong>{money(summary.membershipSurchargeAmount)}</strong></div>
