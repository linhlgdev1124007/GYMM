  ...
  return (
    <>
      <PageHeader
        eyebrow="Phân tích"
        title="Báo cáo"
        description="Doanh thu, check-in và công nợ hỗ trợ quyết định vận hành."
      />
      ...
      <div className="metric-strip grid-cols-4">
        {[
          ["Doanh thu", money(data?.summary.revenue)],
          ["Số phiếu thu", data?.summary.payments || 0],
          ["Hội viên hoạt động", data?.summary.activeMembers || 0],
      ...
      <div className="section-grid">
        ...
        <section>
          <div className="section-header">
            <div>
              <h2>Tổng công nợ</h2>
              <p>Các khoản cần thu</p>
            </div>
          </div>
          <div className="panel">
            <strong className="text-xl font-semibold text-slate-900">
              {money(data?.debts.reduce((sum, row) => sum + row.amount, 0))}
            </strong>
            <p className="mt-1 text-xs text-slate-500">
              {data?.debts.length || 0} gói còn dư nợ
            </p>
          </div>
        </section>
      </div>
      <section className="mt-7">
        <div className="section-header">
          <div>
            ...
            <p>Ưu tiên các khoản đã quá hạn</p>
          </div>
        </div>
        <DataTable
