...
export function GlobalSearch() {
  ...
  const choose = (row) => {
    navigate(`/members?member=${row.id}`);
    setOpen(false);
  };
  return (
    <>
          ...
          >
            <div className="command-input">
              ...
              <input
                ...
                placeholder="Tên, số điện thoại hoặc mã hội viên…"
                onKeyDown={(event) => {
            ...
            <div className="command-results">
              ...
              {items.map((row, index) => (
                ...
                >
                  ...
                  <span>
                    ...
                    <small>
                      {row.code} · {row.phone || "Chưa có SĐT"} ·{" "}
                      {row.membership?.package.name || "Chưa có gói"}
                    </small>
                  </span>
              ...
              {q.length < 2 && (
                <p>
                  Nhập ít nhất 2 ký tự để tìm kiếm. Dùng ↑ ↓ và Enter để thao
                  tác nhanh.
                </p>
              )}
              {q.length >= 2 && !results.isLoading && !items.length && (
                <p>Không tìm thấy hội viên phù hợp.</p>
              )}
            </div>
          </section>
        </div>
